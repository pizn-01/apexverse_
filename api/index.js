...
var PostgresStorage = class {
  ...
  async createContactSubmission(insertSubmission) {
    const [submission] = await this.db.insert(contactSubmissions).values({
  ...
  async getContactSubmission(id) {
    const [submission] = await this.db.select().from(contactSubmissions).where(eq(contactSubmissions.id, id));
    return submission;
  }
  async createTestimonial(insertTestimonial) {
    const [testimonial] = await this.db.insert(testimonials).values({
...
var storage = process.env.DATABASE_URL ? new PostgresStorage(process.env.DATABASE_URL) : new MemStorage();
...
async function handler(req, res) {
  try {
    ...
    if (pathname === "/api/testimonials" && req.method === "GET") {
      ...
      console.log(`[API] Fetched ${testimonials2.length} testimonials`);
      return res.status(200).json(testimonials2);
    }
    ...
    if (pathname === "/api/health" && req.method === "GET") {
      ...
      return res.status(200).json({
        status: "healthy",
        database: "connected",
        testimonialCount: testimonials2.length,
